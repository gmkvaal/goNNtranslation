...
func ValidateArgMaxSlice(n *Network, inputData, outputData []*mat64.Vector) bool {

	var yes, no int

	for i := range inputData {
		if checkIfEqual(n.forwardFeed(inputData[i], 0).RawVector().Data, outputData[i].RawVector().Data) == 1 {
			yes += 1
		} else {
			no += 1
		}
